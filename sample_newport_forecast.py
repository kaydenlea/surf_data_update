#!/usr/bin/env python3
"""
Sample script to extract temperature and atmospheric data for tomorrow
from GFS Atmospheric model for Newport Beach, CA
"""
...
def get_newport_beach_forecast():
    """
    Extract temperature and atmospheric data for tomorrow for Newport Beach.
    Newport Beach coordinates: 33.6189° N, 117.9298° W
    """

    # Newport Beach location
    sample_lat = 33.6189
    sample_lon = -117.9298
    location_name = "Newport Beach, CA"

    logger.info("=" * 80)
    logger.info(f"GFS ATMOSPHERIC FORECAST - {location_name}")
    logger.info("=" * 80)
    ...
    # Extract atmospheric data for Newport Beach
...
if __name__ == "__main__":
    try:
        results = get_newport_beach_forecast()
        if results:
            logger.info("\nForecast extraction completed successfully")
            sys.exit(0)
        else:
            logger.error("\nForecast extraction failed")
            sys.exit(1)
